...
filterSelectedOptions | `Boolean`           | Set to `true` to filter already selected items from the results dropdown. The default is `false`, in which case selected results remain, but may have a CSS class to indicate they're already selected.
